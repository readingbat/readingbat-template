...
- Detekt (`dev.detekt` 2.0.0-alpha.5) and Kotlinter (`org.jmailen.kotlinter` 5.5.0) Gradle plugins for static analysis and ktlint formatting.
...
- **Upgraded the JVM toolchain 17 → 25.** `gradle/libs.versions.toml` sets `jvm = "25"` (read by `build.gradle.kts`). A JDK 25 is now required to build.
- Bumped dependencies: `readingbat-core` 3.1.5 → 3.2.1, Kotlin 2.3.21 → 2.4.0, Ktor 3.4.3 → 3.5.1, Kotest 6.1.11 → 6.2.1, `core-utils` 2.8.2 → 2.9.3, `kotlin-logging` 8.0.02 → 8.0.4, and the Gradle wrapper 9.5.0 → 9.6.1.
- Updated `ContentTests` for readingbat-core's now-suspend `correctAnswers()`, so the answer assertion runs inside a `runBlocking { … }` block.
...
- Tidied `build.gradle.kts`: dropped the redundant `application` plugin (the Ktor plugin already applies it) and gave the `stage` task a `group` and `description`.
